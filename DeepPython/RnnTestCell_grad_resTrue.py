...
class BasicRNNCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        grads = tf.gradients(tf.squeeze(loss_result, axis=0), [home_state, away_state])
        ...
        return tf.expand_dims(loss_result, axis=1), next_state
...
#     print(sess.run(x))
# exit()
loss_train = tf.reduce_sum(loss['train'] * bashs['train']['counted_for_loss'])/tf.reduce_sum(bashs['train']['counted_for_loss'])
# lossr = tf.reduce_mean(loss['train'])  # +  0.001 * (cell['train'].amixer ** 3) * cell['train'].reg
loss_test = tf.reduce_sum(loss['test'] * bashs['test']['counted_for_loss'])/tf.reduce_sum(bashs['test']['counted_for_loss'])
